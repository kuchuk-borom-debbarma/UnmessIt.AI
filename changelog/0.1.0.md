...
- Changelog entries now live in one markdown file per version under `changelog/`, and frontend release metadata is generated from those files in semver order.
...
- Replaced the monolithic changelog source with a root changelog index plus versioned release files.
