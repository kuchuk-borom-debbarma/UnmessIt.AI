...
This means config presets can keep the familiar LM Studio URL from the Settings UI. Docker deployments make it reachable at runtime.
...
5. Resume or recreate failed ingest jobs after changing processing settings or config presets.
...
- Confirm LM Studio is running and serving embeddings for the selected config preset's embedding model.
